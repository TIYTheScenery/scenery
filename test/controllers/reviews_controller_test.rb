...
class ReviewsControllerTest < ActionController::TestCase
  test "create will accept json and create a review" do
    test_input = JSON.parse(File.read("#{Rails.root}/test/fixtures/mock_review_create.json")).merge(format: :json)
    post :create, test_input
    assert_equal Review.last.opinion, ""
  end

  test "create will return errors if review creation fails" do
    test_input = JSON.parse(File.read("#{Rails.root}/test/fixtures/mock_review_create2.json")).merge(format: :json)
    post :create, test_input
  ...
  test "review may not be created unless the user is logged in" do
    test_input = JSON.parse(File.read("#{Rails.root}/test/fixtures/mock_review_create3.json")).merge(format: :json)
    post :create, test_input
    response = JSON.parse(@response.body)
    assert_equal nil, response["success"]
  end
