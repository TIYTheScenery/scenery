...
class CompaniesControllerTest < ActionController::TestCase
  ...
  test "you must be a professional to create a company" do
    test_input = JSON.parse(File.read("#{Rails.root}/test/fixtures/mock_company_create4.json")).merge(format: :json)
    test_input2 = JSON.parse(File.read("#{Rails.root}/test/fixtures/mock_company_create5.json")).merge(format: :json)
    post :create, test_input
    response = JSON.parse(@response.body)
