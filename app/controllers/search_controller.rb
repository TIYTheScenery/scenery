class SearchController < ApplicationController
  def index
    ...
    @performances = Performance.joins("JOIN genre_performances on genre_performances.performance_id = performances.id").
        where("genre_performances.genre_id = ? AND ", params[:genre_id]).
        joins("JOIN show_times ON performances.id = show_times.event_id").
        where("(show_times.event_type = 'Performance' AND (LOWER(show_times.address) LIKE LOWER(?) OR (LOWER(show_times.city) LIKE LOWER(?) AND LOWER(show_times.state) LIKE LOWER(?) ) OR show_times.zip_code LIKE ? )) OR (LOWER(performances.name) LIKE LOWER(?) OR LOWER(performances.description) LIKE LOWER(?) )",
