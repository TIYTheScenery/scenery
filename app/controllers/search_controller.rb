class SearchController < ApplicationController
  def index
    ...
    @performances = Performance.joins("JOIN genre_performances on genre_performances.performance_id = performances.id").
        where("genre_performances.genre_id = ? AND (performances.name LIKE ? OR performances.description LIKE ? ) COLLATE utf8_general_ci", params[:genre_id], "%#{params[:search_term]}%", "%#{params[:search_term]}%" )
