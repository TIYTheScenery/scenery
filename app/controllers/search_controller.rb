class SearchController < ApplicationController
  def index
    ...
    @performances = Performance.joins("JOIN genre_performances on genre_performances.performance_id = performances.id").
        where("genre_performances.genre_id = ? AND (LOWER(performances.name) LIKE LOWER(?) OR LOWER(performances.description) LIKE LOWER(?) )", params[:genre_id], "%#{params[:search_term]}%", "%#{params[:search_term]}%" )
